...
# 기본 설정값
PROJECT_ID="${SUPABASE_PROJECT_ID:-esbjgvnlqzseomhbsimz}"
...
DUMP_FILENAME="convi_daily_backup_${TIMESTAMP}"
...
ln -sf "${DUMP_FILENAME}.sql" "${OUTPUT_DIR}/latest_daily_backup.sql"
ln -sf "${DUMP_FILENAME}.sql.gz" "${OUTPUT_DIR}/latest_daily_backup.sql.gz"
log_success "최신 덤프 링크 생성 완료"

# 이전 백업 파일 정리 (30일 이전 파일 삭제)
log_info "🧹 이전 백업 파일 정리 중..."
find "$OUTPUT_DIR" -name "convi_daily_backup_*.sql*" -mtime +30 -delete
log_success "30일 이전 백업 파일 정리 완료"

# 결과 요약
log_info "=================================="
log_success "✅ 매일 자동 백업 완료!"
log_info "=================================="
log_info "📁 생성된 파일들:"
log_info "  • 원본: ${OUTPUT_DIR}/${DUMP_FILENAME}.sql ($DUMP_SIZE)"
log_info "  • 압축: ${OUTPUT_DIR}/${DUMP_FILENAME}.sql.gz ($COMPRESSED_SIZE)"
log_info "  • 최신: ${OUTPUT_DIR}/latest_daily_backup.sql.gz"
log_info ""
log_info "🔄 다음 백업: 내일 같은 시간"
log_info "=================================="

log_success "🎉 매일 자동 백업이 완료되었습니다!" 